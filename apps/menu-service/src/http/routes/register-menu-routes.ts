...
import { menuCalculationRequestSchema, requiredIdParamSchema, saveMenuSchema } from "@labellens/contracts";

export function registerMenuRoutes(app: Hono<ServiceBindings>, dependencies: MenuServiceDependencies): void {
  ...
  app.post("/api/v1/menus", async (c) => {
    const correlationId = c.get("correlationId");
    const auth = await requireAuthUser(dependencies.authSessionVerifier, c.req.header("Authorization"), correlationId);
